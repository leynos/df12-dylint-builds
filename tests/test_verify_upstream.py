...
UPSTREAM_TARGET = "x86_64-unknown-linux-gnu"
...
def upstream_server(tmp_path: Path) -> Iterator[tuple[str, Path]]:
    ...
    class Handler(http.server.SimpleHTTPRequestHandler):
        def __init__(self, *args: object, **kwargs: object) -> None:
            super().__init__(*args, directory=str(tmp_path / "served"), **kwargs)  # type: ignore[arg-type]
...
# A configuration that builds the target upstream publishes. It exists only
# to manufacture archives shaped exactly like upstream's for the fixture
# server, so the checks below run against something upstream-like.
PUBLISHER_CONFIG = f"""
schema_version = 1
...
def _publish_upstream_fixture(root: Path, tmp_path: Path) -> None:
    """Package archives for the upstream target and serve them as upstream would."""
    pack(
        parse_config(PUBLISHER_CONFIG),
        UPSTREAM_TARGET,
        write_stubs(tmp_path / "release"),
        root,
    )
...
def test_upstreams_archives_pass_our_own_checks(
    ...
    assert len(digests) == len(config.binaries)
